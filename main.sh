...
# Tmux session
tmux set-option -g status-right '#[fg=#{@session}] #(~/.tmux/plugins/tmux-one-dark-minimalist/plugins/session.sh)  '
# Git branch
tmux set-option -g status-left '  #[fg=#{@git}] #(~/.tmux/plugins/tmux-one-dark-minimalist/plugins/git.sh)'
